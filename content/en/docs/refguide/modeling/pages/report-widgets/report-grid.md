...
A **Report grid** shows data retrieved from the database using a [Data set](/refguide/data-sets/) in a grid format. Each row in the grid displays a single result from the data set. Each time a report is created, the data is retrieved from the database.
...
![Report grid in structure mode](/attachments/refguide/modeling/pages/report-widgets/report-grid/report-grid.png)

## 2 Report Grid Properties

An example of report grid properties is represented in the image below:

{{% image_container width="300" %}}![Report grid in structure mode](/attachments/refguide/modeling/pages/report-widgets/report-grid/report-grid-properties.png)
{{% /image_container %}}
...
![Zoom configuration showing columns being passed as parameters](/attachments/refguide/modeling/pages/report-widgets/report-grid/report-zoom.png)
...
* by dragging the border between the columns:
    ...
    ![Drag column widths](/attachments/refguide/modeling/pages/report-widgets/report-grid/drag-column-width.png)

* by entering the percentages explicitly

    ![Enter column widths](/attachments/refguide/modeling/pages/report-widgets/report-grid/enter-column-widths.png)
...
![Add the Export to Excel button](/attachments/refguide/modeling/pages/report-widgets/report-grid/export-to-excel.png)
...
If **Generate on page load** is set to **No**, the report grid will not show any data until the end-user presses the [Generate report button](/refguide/report-button/). This is especially useful if the report uses parameters that should first be specified by the end-user. 

## 3 Report Grid Column Properties{#column-properties}

An example of report grid properties is represented in the image below:

{{% image_container width="250" %}}![Report grid column properties](/attachments/refguide/modeling/pages/report-widgets/report-grid/report-grid-column-properties.png)
{{% /image_container %}}
...
![Drag value from Connector pane into a column](/attachments/refguide/modeling/pages/report-widgets/report-grid/drag-column-value.png)

...
Another way to create a new action is to extract an action out of test steps from a test case. For more information on how to extract an action, see the "Extract Action, Copy, Paste, Delete" section of [Test Case Reference](/addons/ats-addon/rg-two-test-case/).
...
{{< figure src="/attachments/addons/ats-addon/rg-ats/rg-two-ats/rg-two-action/action.png" alt="" >}}

#### 2.2.1 Input Parameters

An input parameter is a value that you use inside your action. The input parameter is shown as an [action parameter](/addons/ats-addon/rg-two-test-step/#action-parameter) of a test step. An action can have multiple optional or required input parameters, but it can only have one output parameter.
